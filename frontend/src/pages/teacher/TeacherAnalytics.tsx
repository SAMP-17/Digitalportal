...
const TeacherAnalytics = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const myTests = tRes.data || [];
      const myTestIds = new Set(myTests.map((t: any) => t.id));
      setTests(myTests);
      setAttempts((aRes.data || []).filter((a: any) => myTestIds.has(a.test_id)));
      setQuestions((qRes.data || []).filter((q: any) => myTestIds.has(q.test_id)));
      setResponses((rRes.data || []).filter((r: any) => myTestIds.has(r.questions?.test_id)));
      setLoading(false);
    });
  }, [user]);
  ...
  const overallPassRate = attempts.length
    ? Math.round((attempts.filter(a => Number(a.score) >= 50).length / attempts.length) * 100)
    : 0;
