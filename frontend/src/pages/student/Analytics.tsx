...
const Analytics = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setResponses(respRes.data?.filter((r: any) => r.test_attempts) || []);
      setClassAttempts(classRes.data || []);
      setLoading(false);
    });
  }, [user]);

  // === Performance Trend ===
  const trendData = useMemo(() => attempts.map((a: any, i: number) => ({
    attempt: i + 1,
  ...
  const timeData = useMemo(() => {
    ...
    attempts.forEach((a: any) => {
      ...
      const questionCount = responses.filter((r: any) => r.test_attempts?.test_id === a.test_id).length || 1;
      attemptTimes.push({
  ...
  const classComparisonData = useMemo(() => {
    ...
    return Object.entries(testGroups).map(([testId, data]) => {
      const myAttempt = attempts.find((a: any) => a.test_id === testId);
                  ...
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <ScatterChart>
                        ...
                        <Tooltip
                          contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "var(--radius)" }}
                          formatter={(value: any, name: string) => [name === "score" ? `${value}%` : `${value} min`, name === "score" ? "Score" : "Time/Q"]}
                        />
